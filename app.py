...
def valuate_crypto_for_2023(crypto):
    ...
    while current_date <= end_date:
        ...
        print(f"Valuating {crypto} for {current_date.strftime('%d-%m-%Y')}...")
        price = get_datetime_binance_price(crypto, datetime_str)
        if price != "No data available for the specified time":
            ...
            api_valuate_deals(crypto, current_date.strftime("%d-%m-%Y"), float(price))
        else:
            print(price)
        current_date += delta
